# MTPADMIN 0.12.0 public client UI layer.
# Presentation only: keeps the proven handlers/actions underneath and makes the
# panel friendlier for normal VPS owners on desktop, tablet and mobile.
...
body.mtp-modern .user{margin-left:auto;display:flex;align-items:center;gap:10px}.m-user-dot{width:34px;height:34px;border-radius:50%;display:grid;place-items:center;border:1px solid var(--line2);background:#142238;color:#d7e3f4;font-weight:700;text-transform:uppercase}.m-user-name{color:#9fb0c7;font-size:12px}.desktop-tech{display:none!important}
...
body.mtp-modern table{font-size:12.5px}body.mtp-modern th{color:#91a4bd!important;font-size:11px;font-weight:650;text-transform:none;letter-spacing:.15px;background:rgba(18,32,51,.58)}body.mtp-modern th,body.mtp-modern td{padding:10px 9px!important;border-bottom-color:#1d3049!important}body.mtp-modern tr:hover td{background:rgba(37,131,255,.035)!important}
...
.m-mobile-menu{display:none}.m-mobile-nav{display:none}
...
(function(){
 function enhance(){
  ...
  if(nav&&!nav.querySelector('.m-side-links')){
...
def page_template(title,body,user,active='dashboard',refresh=None,message=''):
    ...
    footer=(f"<div class=\"footer\"><span>© {year} MTPADMIN · часть экосистемы <a href=\"{_PUBLIC_SITE}\" target=\"_blank\" rel=\"noopener\">VPN BOSS</a></span>"
            f"<span class=\"m-footer-links\"><a href=\"{_PUBLIC_GITHUB}\" target=\"_blank\" rel=\"noopener\">Исходный код</a><a href=\"{_PUBLIC_TG}\" target=\"_blank\" rel=\"noopener\">Помощь и сообщество</a><span>версия {esc(VERSION)}</span></span></div>")
...
def dashboard_html():
    ...
    ips=sum(int(u.get('active_unique_ips') or 0) for u in users)
    score=100
    ...
    traffic=scalar("SELECT coalesce(sum(bytes_from_client+bytes_to_client),0) AS n FROM daily_traffic WHERE day=?",(today,),0)
    updates=0
    ...
    metrics=f"""<div class='grid' style='margin-top:12px'>
<div class='card metric'><div class='k'>Состояние</div><div class='v {'ok' if good else 'warn'}'>{score}/100</div><div class='muted'>общая проверка</div></div>
<div class='card metric'><div class='k'>Подключения сейчас</div><div class='v'>{current}</div><div class='muted'>активных IP: {ips}</div></div>
<div class='card metric'><div class='k'>Клиенты сегодня</div><div class='v'>{esc(uniq)}</div><div class='muted'>трафик: {esc(human_bytes(traffic))}</div></div>
<div class='card metric'><div class='k'>WEB Proxy</div><div class='v {'ok' if wp_ready else 'warn'}'>{'Готов' if wp_ready else 'Проверить'}</div><div class='muted'>{esc(wp_host)}</div></div>
</div>"""
    quick="""<div class='m-section-title'><div><h2>Быстрые действия</h2><p>Самые частые действия — без поиска по меню</p></div></div><div class='m-quick'>
<a href='/links'><i>↗</i><span><b>Ссылки</b><br><small class='muted'>Подключить Telegram</small></span></a>
<a href='/active'><i>◉</i><span><b>Активные клиенты</b><br><small class='muted'>Кто подключён сейчас</small></span></a>
<a href='/sources'><i>＋</i><span><b>Источники</b><br><small class='muted'>Создать отдельную ссылку</small></span></a>
<a href='/operations'><i>✓</i><span><b>Проверить систему</b><br><small class='muted'>Обновления и состояние</small></span></a>
</div>"""
    source_rows_ui=[]
    for u in users[:10]:
        name=str(u.get('username') or '')
        source_rows_ui.append([esc(name),'<span class="ok">● работает</span>' if u.get('enabled',True) else '<span class="bad">● выключен</span>',esc(u.get('current_connections',0)),esc(u.get('active_unique_ips',0)),esc(human_bytes(u.get('total_octets',0)))])
    sources=f"""<div class='m-section-title'><div><h2>Источники подключения</h2><p>Краткая сводка по вашим ссылкам</p></div><a href='/sources'>Все источники →</a></div><div class='card'>{table(['Источник','Состояние','Подключения','Активные IP','Трафик'],source_rows_ui)}</div>"""
...
def _o_update_center(csrf):
    ...
    for key,label in labels:
        ...
        button=f"<form class='inline' method='post' action='/action/component-update/{esc(key)}'><input type='hidden' name='csrf' value='{esc(csrf)}'><button {'class=\"warnbtn\"' if available else 'class=\"secondary\"'}>{text}</button></form>"
    ...
    if cs:
        st_name=str(cs.get('state') or ''); cls='ok' if st_name=='success' else ('bad' if st_name=='failed' else 'warn')
        human={'success':'успешно','failed':'ошибка','running':'выполняется','queued':'в очереди'}.get(st_name,st_name)
...
def _o_webproxy_card(csrf):
    ...
    qr=_o_qr(link); status='<span class="ok">● Готов к работе</span>' if ready else '<span class="warn">● Требуется проверка</span>'
...
def _o_operations_html(user,csrf):
    score,checks=_o_health(); st=_o_state(); current=st.get('MTPADMIN_VERSION',VERSION); updates=int((_o_update_status().get('updates') or 0))
...
<div class='card metric'><div class='k'>Состояние сервиса</div><div class='v {'ok' if score>=90 else 'warn'}'>{score}/100</div><div class='muted'>{'всё работает' if score>=90 else 'нужна проверка'}</div></div>
<div class='card metric'><div class='k'>Версия MTPADMIN</div><div class='v'>{esc(current)}</div><div class='muted'>установлена сейчас</div></div>
<div class='card metric'><div class='k'>WEB Proxy</div><div class='v {'ok' if wp_ready else 'warn'}'>{'Готов' if wp_ready else 'Проверить'}</div><div class='muted'>{esc(st.get('WEBPROXY_HOST','не настроен'))}</div></div>
<div class='card metric'><div class='k'>Обновления</div><div class='v {'warn' if updates else 'ok'}'>{updates}</div><div class='muted'>{'доступно' if updates else 'всё актуально'}</div></div></div>"""
