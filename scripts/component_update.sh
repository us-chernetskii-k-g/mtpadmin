...
ROOT='https://raw.githubusercontent.com/us-chernetskii-k-g/mtpadmin/main'
TMP=''
...
data={'component':comp,'state':state,'detail':detail,'ts':int(time.time())}
fd,tmp=tempfile.mkstemp(prefix='.component-update.',dir=os.path.dirname(p),text=True)
with os.fdopen(fd,'w') as f: json.dump(data,f,ensure_ascii=False); f.write('\n'); f.flush(); os.fsync(f.fileno())
...
update_telemt(){
  ...
  TMP=$(mktemp -d)
  local arch libc latest cur asset base archive sumurl expected actual backup
  ...
  base="telemt-${arch}-linux-${libc}.tar.gz"
  archive="$TMP/$base"; sumurl="https://github.com/telemt/telemt/releases/download/$latest/$base.sha256"
  curl -fL --retry 3 "https://github.com/telemt/telemt/releases/download/$latest/$base" -o "$archive"
  expected=$(curl -fsSL --retry 3 "$sumurl" | awk '{print $1}' | head -1)
  actual=$(sha256sum "$archive" | awk '{print $1}')
  [[ "$expected" =~ ^[0-9a-f]{64}$ && "$actual" == "$expected" ]] || die 'Checksum нового TeleMT не совпал'
  tar -xzf "$archive" -C "$TMP"
  local bin; bin=$(find "$TMP" -type f -name telemt -print -quit); [[ -n "$bin" ]] || die 'В release archive нет telemt'
  chmod 0755 "$bin"; "$bin" --help >/dev/null 2>&1 || die 'Новый TeleMT binary не запускается'
  install -d -m 0700 /var/backups/mtpadmin
  backup="/var/backups/mtpadmin/telemt-before-${latest}-$(date +%Y%m%d-%H%M%S)"
  cp -a /usr/local/bin/telemt "$backup"
  log_status "$COMPONENT" running "Устанавливаю TeleMT $latest; будет короткий рестарт proxy"
  install -m 0755 -o root -g root "$bin" /usr/local/bin/telemt
  systemctl restart mtpadmin-telemt.service
  if ! wait_telemt; then
    cp -a "$backup" /usr/local/bin/telemt
    systemctl restart mtpadmin-telemt.service || true
    wait_telemt || true
    die "TeleMT $latest не вышел в READY; восстановлен предыдущий binary"
  fi
  "$CHECKER" >/dev/null 2>&1 || true
  log_status "$COMPONENT" success "TeleMT обновлён ${cur:-?} → $latest"
  ok "TeleMT обновлён ${cur:-?} → $latest"
}
...
update_webproxy(){
  ...
  "$CHECKER" >/dev/null 2>&1 || true
  log_status "$COMPONENT" success "WEB Proxy обновлён ${cur:0:12} → ${latest:0:12}"
  ok "WEB Proxy обновлён → ${latest:0:12}"
}
...
set_webproxy_host(){
  ...
  WEBPROXY_HOST_OVERRIDE="$host" bash "$WEBINSTALL"
  "$CHECKER" >/dev/null 2>&1 || true
  log_status "$COMPONENT" success "WEB Proxy hostname: $host"
  ok "WEB Proxy hostname: $host"
}

update_mtpadmin(){
  COMPONENT='mtpadmin'; log_status "$COMPONENT" running 'Запущено бесшовное обновление MTPADMIN'
  TMP=$(mktemp -d)
  curl -fsSL --retry 3 "$ROOT/update.sh" -o "$TMP/update.sh" || die 'Не удалось скачать update.sh'
  chmod 0700 "$TMP/update.sh"; bash -n "$TMP/update.sh" || die 'update.sh syntax invalid'
  bash "$TMP/update.sh"
  [[ -x "$CHECKER" ]] && "$CHECKER" >/dev/null 2>&1 || true
  log_status "$COMPONENT" success 'MTPADMIN обновлён'
}
...
dispatch(){
  ...
  systemd-run --quiet --collect --unit="$unit" --property=Type=oneshot --property=TimeoutStartSec=1800 \
    /usr/local/lib/mtpadmin/component_update.sh "$component" "$@"
  echo "$unit"
}
