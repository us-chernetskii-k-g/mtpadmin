...
Главное изменение этой версии — удобная установка веб-панели на телефон как отдельного приложения. При первом входе MTPADMIN предлагает установку, умеет вызвать штатное окно Android, показывает инструкцию для iPhone и позволяет продолжить работу в обычном браузере без постоянных повторных предложений.

Приложение работает только с актуальными данными сервера: страницы админки и статистика специально не кэшируются для автономного режима. Одновременно сохранены новый интерфейс 0.12.x, защита от цикла отрисовки, общий список MTProto и WEB-клиентов, сохранение существующей WEB-ссылки и проверки Центра обновлений.
