...
**Ссылки** — готовые MTProxy и WEB Proxy ссылки с QR.

**Безопасность** — Scanner Guard, Risk score, whitelist и ручные блокировки.

**События** — история важных изменений и действий.

**Операции** — Health, Update Center, WEB Proxy, Guard Learning и backup. В 0.11.4 эта страница стала компактной: подробности разнесены по вкладкам, поэтому нужное видно без длинной прокрутки.

**Система** — состояние TeleMT, collector, Caddy, памяти, диска и других компонентов.

Большие таблицы имеют собственный скролл и sticky-заголовки, а данные обновляются в фоне без постоянной перезагрузки страницы.
...
Hostname можно задать прямо в панели, например:

```text
webproxy.example.com
```

Для него нужна DNS A-запись на IP сервера. Caddy после этого получает TLS-сертификат автоматически.

WEB Proxy использует отдельный источник `WEB_PROXY`, поэтому его статистика не смешивается с обычными ссылками.

## Update Center

Панель проверяет:

- новую версию **MTPADMIN**;
- новые релизы **TeleMT**;
- актуальный upstream commit **Telegram WEB Proxy**.

Если обновление доступно, появляется уведомление. MTPADMIN можно обновить прямо из браузера. Начиная с 0.11.4 веб-обновление привязывается к конкретному commit SHA проверенного `main`, поэтому update-job не зависит от stale-cache плавающей ветки.

TeleMT перед заменой бинарника резервируется и откатывается при неудачном READY-check. WEB Proxy сначала собирается и проверяется, а затем заменяет рабочую версию.
...
**0.11.4** — Compact UI, commit-pinned обновление из веб-панели и корректный запуск upstream permission-тестов Telegram WEB Proxy при защищённом `umask 077` production installer-а.
