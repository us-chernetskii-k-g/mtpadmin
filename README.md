...
**MTPADMIN помогает за несколько минут поднять свой прокси для Telegram на обычном сервере и потом управлять им через понятную веб-панель.**
...
Для веб-панели и WEB Proxy должны быть доступны порты `80` и `443`. Для обычного MTProto нужно открыть выбранный порт, по умолчанию удобно использовать `8443`.
...
Дальше появится мастер установки. Он сам спросит:
...
- пароль веб-панели;
- дополнительные параметры, если они нужны.
...
Основной способ — через веб-панель:

**Операции → Обновления**

MTPADMIN отдельно показывает обновления для:

- самой панели;
- TeleMT;
- Telegram WEB Proxy.

Обновление продолжится даже если закрыть вкладку браузера.
...
На реальном сервере уже проверены:

- обновление самого MTPADMIN;
- обновление TeleMT;
- обновление Telegram WEB Proxy;
- сохранение рабочей WEB-ссылки;
- переход на новый постоянный ключ WEB Proxy;
- восстановление статистики WEB Proxy после обновления;
- правильное отображение версий в Центре обновлений;
- итоговая проверка `mtpadmin doctor` без ошибок.
