...
Обновления запускаются отдельными systemd-задачами и не зависят от открытой вкладки браузера. В панели виден статус `QUEUED → RUNNING → SUCCESS/FAILED`, а кнопки подписаны названием компонента, чтобы нельзя было перепутать обновление MTPADMIN и WEB Proxy.
...
**0.11.5** — асинхронные обновления из веб-панели без HTTP timeout, понятные кнопки для каждого компонента и статус выполнения задачи прямо в Update Center.
