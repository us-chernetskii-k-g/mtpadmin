...
[[ ! -e "$STATE" ]] || die 'MTPADMIN уже установлен. Используйте Update Center или update.sh.'
...
cat >&3 <<'BANNER'
╔════════════════════════════════════════════════════╗
║              MTPADMIN CLEAN INSTALL               ║
║                  setup wizard                     ║
╚════════════════════════════════════════════════════╝

Сначала будут собраны все параметры. До подтверждения система не изменяется.
Для HTTPS заранее создайте DNS A/AAAA записи доменов на этот сервер.
...
PROFILE="${MTP_PROFILE:-$(ask 'Имя первого источника/source' 'MAIN')}"
FAKE_TLS_DOMAIN=$(normal_host "${MTP_FAKE_TLS_DOMAIN:-$(ask 'Fake-TLS SNI домен' 'google.com')}")
...
RAW_SECRET_MODE='provided'
if [[ -z "$RAW_SECRET" ]]; then
  RAW_SECRET=$(ask_secret 'Существующий raw secret 32 hex [Enter = сгенерировать]: ')
  RAW_SECRET_MODE='provided'
fi
if [[ -z "$RAW_SECRET" ]]; then RAW_SECRET=$(od -An -N16 -tx1 /dev/urandom | tr -d ' \n'); RAW_SECRET_MODE='generated'; fi
RAW_SECRET=${RAW_SECRET,,}

if [[ -n "${MTP_AD_TAG+x}" ]]; then AD_TAG=${MTP_AD_TAG,,}; else AD_TAG=$(ask 'Advertising tag @MTProxyBot 32 hex (необязательно)' ''); AD_TAG=${AD_TAG,,}; fi
...
[[ "$PROFILE" =~ ^[A-Za-z0-9_.-]{1,64}$ ]] || die 'Имя source: A-Z a-z 0-9 _ . - до 64 символов.'
[[ "$RAW_SECRET" =~ ^[0-9a-f]{32}$ ]] || die 'Raw secret должен быть ровно 32 hex.'
[[ -z "$AD_TAG" || "$AD_TAG" =~ ^[0-9a-f]{32}$ ]] || die 'Ad tag должен быть пустым или 32 hex.'
[[ "$RETENTION_DAYS" =~ ^[0-9]+$ ]] && ((RETENTION_DAYS>=1 && RETENTION_DAYS<=365)) || die 'Полные IP: 1..365 дней.'
[[ "$ANON_RETENTION_DAYS" =~ ^[0-9]+$ ]] && ((ANON_RETENTION_DAYS>=RETENTION_DAYS && ANON_RETENTION_DAYS<=3650)) || die 'Обезличенная история должна быть >= raw retention и <=3650.'
...
Public/NAT IPv4:     $PUBLIC_IP
Первый source:       $PROFILE
FakeTLS SNI:         $FAKE_TLS_DOMAIN
Raw secret:          $RAW_SECRET_MODE (значение скрыто)
Advertising tag:     $([[ -n "$AD_TAG" ]] && echo SET || echo 'не задан')
Promoted channel:    ${PROMOTED_CHANNEL:-не задан}
Полные IP:           $RETENTION_DAYS дней
Anon history:        $ANON_RETENTION_DAYS дней

Веб-админка:         https://$WEB_HOST/
Логин:               $WEB_USER
Пароль:              SET (значение скрыто)
Telegram WEB Proxy:  https://$WEBPROXY_HOST/

Release:             $VERSION · ${RELEASE_REF:0:12}
Build filesystem:    $(df -P "$BUILD_ROOT" | awk 'NR==2{print $1}') · $((FREE_BUILD_KB/1024)) MB free
─────────────────────────────────────────────────────
EOF
...
[[ -f /etc/caddy/Caddyfile ]] || die 'Caddy установлен, но Caddyfile отсутствует.'
ok 'Caddy готов'
...
curl -fsSL --retry 3 "$ROOT/$RELEASE_REF/web-install.sh" -o "$TMP/web-install.sh" || die 'Не удалось скачать pinned web-install.sh.'
...
ok "MTPADMIN $VERSION clean install завершён: TeleMT + Web + WEB Proxy + telemetry + Update Center."
