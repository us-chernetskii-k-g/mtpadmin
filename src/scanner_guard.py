#!/usr/bin/env python3
import argparse, ipaddress, json, re, signal, sqlite3, subprocess, sys, time, urllib.request
from pathlib import Path

VERSION='0.7.0'
STATE=Path('/etc/mtpadmin/state.env')
DB=Path('/var/lib/mtpadmin/stats.db')
...
def nset(ip,temp):
    return f"blocked{ipaddress.ip_address(ip).version}_{'temp' if temp else 'perm'}"


def script(s,check=True):
    return nft('-f','-',input=s,check=check)


def delban_nft(ip):
    a=str(ipaddress.ip_address(ip))
    for t in (False,True):
        script(f'delete element inet {TABLE} {nset(a,t)} {{ {a} }}\n',False)
...
def addban_nft(ip,exp=None):
    ...
    if exp:
        ...
        script(f'add element inet {TABLE} {name} {{ {a} }}\n')
...
def sync_bans():
    ...
    for r in rs:
        try:
            addban_nft(r['ip'],r['expires_at'])
        except Exception as e:
            print(f'restore ban {r["ip"]}: {e}',file=sys.stderr)


def ensure(port,force=False):
    initdb()
    exists=nft('list','table','inet',TABLE,check=False).returncode==0
    with con() as c:
        r=c.execute("SELECT value FROM scanner_meta WHERE key='guard_port'").fetchone(); old=r[0] if r else None
    if exists and old==str(port) and not force:
        return
    nft('delete','table','inet',TABLE,check=False)
    script(rules(port))
    with con() as c:
        c.execute("INSERT INTO scanner_meta VALUES('guard_port',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value",(str(port),))
    sync_bans()
    ...
    for o in d.get('nftables',[]):
        ...
        for x in s.get('elem') or []:
            if isinstance(x,str):
                ...
                if isinstance(v,str):
                    out[v]=int((e.get('counter') or {}).get('packets') or 1)
    return out
...
def valid_info():
    """Return trusted TeleMT client IPs mapped to their best known validation time."""
    ...
    try:
        ...
        with con() as c:
            for r in c.execute("SELECT ip,last_seen FROM clients WHERE ip IS NOT NULL AND ip!='' AND last_seen>=?",(cutoff,)):
                try:
                    ip=str(ipaddress.ip_address(r[0])); out[ip]=int(r[1] or now)
    ...
    try:
        for u in api('/v1/users') or []:
            for k in ('active_unique_ips_list','recent_unique_ips_list'):
                for x in u.get(k) or []:
                    try:
                        ip=str(ipaddress.ip_address(x)); out.setdefault(ip,now)
...
def valid():
    return set(valid_info())


def geo(ip):
    try:
        with con() as c:
            r=c.execute('SELECT country_code,city,asn,org FROM clients WHERE ip=?',(ip,)).fetchone()
        if r and any(r):
            return tuple((r[k] or '') for k in ('country_code','city','asn','org'))
    except Exception:
        pass
    cc=city=asn=org=''
    try:
        import maxminddb
        if CITY.exists():
            with maxminddb.open_database(str(CITY)) as rd:
                q=rd.get(ip) or {}
            cc=(q.get('country') or {}).get('iso_code') or ''
            city=((q.get('city') or {}).get('names') or {}).get('en') or ''
        if ASN.exists():
            with maxminddb.open_database(str(ASN)) as rd:
                q=rd.get(ip) or {}
            n=q.get('autonomous_system_number') or q.get('asn') or q.get('as_number')
            org=q.get('autonomous_system_organization') or q.get('as_organization') or ''
            if n:
                asn=str(n).upper(); asn=asn if asn.startswith('AS') else 'AS'+asn
    except Exception:
        pass
...
def hosting(org):
    return any(x in (org or '').lower() for x in HOSTING)
...
def _get(r,key,default=None):
    try:
        ...
        v=getattr(r,key,default) if not isinstance(r,dict) else r.get(key,default)
...
def risk_score(r,now=None):
    """Heuristic review score only. It is never an automatic ban trigger."""
    now=int(now or time.time())
    cls=str(_get(r,'classification','UNKNOWN') or 'UNKNOWN')
    if cls in ('CLIENT','WHITELIST'): return 0
    if cls=='BANNED': return 100
    if int(_get(r,'valid_ever',0) or 0): return 0
    n=max(0,int(_get(r,'total_attempts',0) or 0))
    first=int(_get(r,'first_seen',now) or now); last=int(_get(r,'last_seen',first) or first)
    age=max(0,last-first)
    score=min(45,n*5)
    if n>=3: score+=5
    if n>=5: score+=10
    if n>=10: score+=10
...
def transition(c,row,new_class,now,detail='observer'):
    old=str(_get(row,'classification','UNKNOWN') or 'UNKNOWN')
    risk=risk_score({**dict(row),'classification':new_class},now)
    ip=str(_get(row,'ip',''))
    c.execute('UPDATE scanner_observations SET classification=?,risk_score=? WHERE ip=?',(new_class,risk,ip))
    if new_class!=old:
        c.execute('INSERT INTO scanner_state_events(ts,ip,old_class,new_class,attempts,risk_score,detail) VALUES(?,?,?,?,?,?,?)',
                  (now,ip,old,new_class,int(_get(row,'total_attempts',0) or 0),risk,detail[:120]))
    return risk
...
def set_class(ip,new_class,detail):
    ...
    with con() as c:
        ...
        if row:
            transition(c,row,new_class,now,detail)
...
def collect():
    ...
    with con() as c:
        c.execute('UPDATE scanner_bans SET active=0,removed_at=? WHERE active=1 AND expires_at IS NOT NULL AND expires_at<=?',(now,now))
        bans={r[0] for r in c.execute('SELECT ip FROM scanner_bans WHERE active=1 AND (expires_at IS NULL OR expires_at>?)',(now,))}
        white={r[0] for r in c.execute('SELECT ip FROM scanner_whitelist')}
        for ip,count in ss.items():
            ...
            if r:
                prev=int(r['last_nft_counter']); delta=count-prev if count>=prev else count
                cc,city,asn,org=(r['country_code'],r['city'],r['asn'],r['org'])
                if not any((cc,city,asn,org)): cc,city,asn,org=geo(ip)
                c.execute('''UPDATE scanner_observations SET last_seen=?,total_attempts=total_attempts+?,last_nft_counter=?,
                  valid_ever=CASE WHEN ? THEN 1 ELSE valid_ever END,
                  valid_last_seen=CASE WHEN ? THEN max(coalesce(valid_last_seen,0),?) ELSE valid_last_seen END,country_code=?,city=?,asn=?,org=? WHERE ip=?''',
                  (now,max(0,delta),count,ip in vv,ip in vv,int(vv.get(ip,now)),cc,city,asn,org,ip))
            else:
                cc,city,asn,org=geo(ip)
                c.execute('''INSERT INTO scanner_observations(ip,family,first_seen,last_seen,total_attempts,last_nft_counter,valid_ever,valid_last_seen,country_code,city,asn,org)
                  VALUES(?,?,?,?,?,?,?,?,?,?,?,?)''',(ip,fam,now,now,max(count,1),count,1 if ip in vv else 0,int(vv[ip]) if ip in vv else None,cc,city,asn,org))
        for ip in vv:
            r=c.execute('SELECT ip FROM scanner_observations WHERE ip=?',(ip,)).fetchone()
            if r:
                c.execute('UPDATE scanner_observations SET valid_ever=1,valid_last_seen=max(coalesce(valid_last_seen,0),?) WHERE ip=?',(int(vv[ip]),ip))
            else:
                fam=ipaddress.ip_address(ip).version; cc,city,asn,org=geo(ip)
                c.execute('''INSERT INTO scanner_observations(ip,family,first_seen,last_seen,valid_ever,valid_last_seen,country_code,city,asn,org)
                  VALUES(?,?,?,?,1,?,?,?,?,?)''',(ip,fam,now,now,int(vv[ip]),cc,city,asn,org))
        rows=c.execute('SELECT * FROM scanner_observations WHERE last_seen>=?',(now-30*86400,)).fetchall()
        for r in rows:
            transition(c,r,classify(r,bans,white,now),now)
        c.execute("INSERT INTO scanner_meta VALUES('heartbeat',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value",(str(now),))
        c.execute("INSERT INTO scanner_meta VALUES('autoban','0') ON CONFLICT(key) DO UPDATE SET value='0'")


def audit(action,ip='',detail='',actor='cli'):
    with con() as c:
        c.execute('INSERT INTO scanner_audit(ts,actor,action,ip,detail) VALUES(?,?,?,?,?)',(int(time.time()),actor[:80],action[:40],ip[:64],detail[:500]))
...
def ban(ip,dur,reason,actor):
    ...
    with con() as c:
        if c.execute('SELECT 1 FROM scanner_whitelist WHERE ip=?',(ip,)).fetchone():
            raise ValueError('IP в whitelist; сначала удалите его из whitelist')
        c.execute('''INSERT INTO scanner_bans VALUES(?,?,?,?,?,1,NULL) ON CONFLICT(ip) DO UPDATE SET
          created_at=excluded.created_at,expires_at=excluded.expires_at,reason=excluded.reason,actor=excluded.actor,active=1,removed_at=NULL''',
          (ip,now,exp,reason[:200],actor[:80]))
    ensure(int(state().get('PORT','8443'))); addban_nft(ip,exp); set_class(ip,'BANNED','manual-ban')
    audit('ban',ip,f'duration={label}; reason={reason}',actor); print(f'BANNED {ip} {label}')


def unban(ip,actor):
    ip=target(ip); delban_nft(ip); now=int(time.time())
    with con() as c:
        c.execute('UPDATE scanner_bans SET active=0,removed_at=? WHERE ip=?',(now,ip))
        row=c.execute('SELECT * FROM scanner_observations WHERE ip=?',(ip,)).fetchone()
        white=bool(c.execute('SELECT 1 FROM scanner_whitelist WHERE ip=?',(ip,)).fetchone())
        if row:
            new='WHITELIST' if white else ('CLIENT' if row['valid_ever'] else 'UNKNOWN')
            transition(c,row,new,now,'manual-unban')
...
def white(ip,note,actor):
    ...
    with con() as c:
        c.execute('UPDATE scanner_bans SET active=0,removed_at=? WHERE ip=?',(now,ip))
        c.execute('INSERT INTO scanner_whitelist VALUES(?,?,?,?) ON CONFLICT(ip) DO UPDATE SET note=excluded.note,actor=excluded.actor',(ip,now,note[:200],actor[:80]))
        row=c.execute('SELECT * FROM scanner_observations WHERE ip=?',(ip,)).fetchone()
...
def unwhite(ip,actor):
    ...
    with con() as c:
        c.execute('DELETE FROM scanner_whitelist WHERE ip=?',(ip,))
        row=c.execute('SELECT * FROM scanner_observations WHERE ip=?',(ip,)).fetchone()
        banned=bool(c.execute('SELECT 1 FROM scanner_bans WHERE ip=? AND active=1 AND (expires_at IS NULL OR expires_at>?)',(ip,now)).fetchone())
        if row:
            new='BANNED' if banned else ('CLIENT' if row['valid_ever'] else 'UNKNOWN')
            transition(c,row,new,now,'manual-unwhitelist')
...
def status(js=False):
    ...
    with con() as c:
        h=c.execute("SELECT value FROM scanner_meta WHERE key='heartbeat'").fetchone()
        cls={r[0]:r[1] for r in c.execute('SELECT classification,count(*) FROM scanner_observations WHERE last_seen>=? GROUP BY classification',(now-86400,))}
        bans=c.execute('SELECT count(*) FROM scanner_bans WHERE active=1 AND (expires_at IS NULL OR expires_at>?)',(now,)).fetchone()[0]
        wh=c.execute('SELECT count(*) FROM scanner_whitelist').fetchone()[0]
        high=c.execute("SELECT count(*) FROM scanner_observations WHERE last_seen>=? AND risk_score>=70 AND classification NOT IN ('CLIENT','WHITELIST')",(now-86400,)).fetchone()[0]
...
def listing(kind):
    ...
    with con() as c:
        if kind=='bans':
            rs=c.execute('SELECT ip,expires_at,reason,actor FROM scanner_bans WHERE active=1 AND (expires_at IS NULL OR expires_at>?)',(int(time.time()),)).fetchall()
        elif kind=='events':
            rs=c.execute('SELECT ts,ip,old_class,new_class,attempts,risk_score,detail FROM scanner_state_events ORDER BY ts DESC LIMIT 100').fetchall()
        elif kind=='observed':
            rs=c.execute('SELECT ip,classification,risk_score,total_attempts,last_seen,valid_last_seen,country_code,city,asn,org FROM scanner_observations ORDER BY last_seen DESC LIMIT 200').fetchall()
        else:
            rs=c.execute("SELECT ip,classification,risk_score,total_attempts,last_seen,country_code,city,asn,org FROM scanner_observations WHERE classification IN ('SCAN','HOSTING?','UNKNOWN') ORDER BY risk_score DESC,total_attempts DESC LIMIT 100").fetchall()
    for r in rs:
...
def worker():
    ...
    while not STOP:
        try:
            ...
            print(f'scanner cycle: {type(e).__name__}: {e}',file=sys.stderr)
        for _ in range(10):
            if STOP: break
            time.sleep(1)
...
def main():
    ...
    s.add_parser('selftest'); s.add_parser('init'); s.add_parser('worker')
    q=s.add_parser('status'); q.add_argument('--json',action='store_true')
    s.add_parser('suspicious'); s.add_parser('bans'); s.add_parser('observed'); s.add_parser('events')
    ...
    q=s.add_parser('unban'); q.add_argument('ip'); q.add_argument('--actor',default='cli')
    q=s.add_parser('whitelist'); q.add_argument('ip'); q.add_argument('--note',default='manual'); q.add_argument('--actor',default='cli')
    q=s.add_parser('unwhitelist'); q.add_argument('ip'); q.add_argument('--actor',default='cli')
    a=p.parse_args()
