...
Ниже — реальные снимки работающей установки MTPADMIN. После выхода 0.12.0 они будут обновлены снимками нового оформления.
...
Главное изменение этой версии — полностью переработанная веб-панель. Интерфейс стал адаптивным, основные действия и показатели вынесены на первый план, добавлены ссылки на GitHub и сообщество, а технические сведения убраны из обычного пользовательского режима в раздел «Для специалистов».

При этом серверная логика 0.11.15 сохранена: обычный MTProto и WEB Proxy продолжают работать вместе, активные WEB-клиенты отображаются в общем списке, существующая WEB-ссылка сохраняется при обновлениях, а Центр обновлений по-прежнему выполняет проверку результата и восстановление при ошибках.
