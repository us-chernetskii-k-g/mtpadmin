...
def state():
    ...
    for x in STATE.read_text().splitlines():
        if '=' not in x or x.lstrip().startswith('#'): continue
        k,v=x.split('=',1); v=v.strip(); d[k.strip()]=v[1:-1] if len(v)>1 and v[0]==v[-1] and v[0] in "'\"" else v
    return d
...
CREATE TABLE IF NOT EXISTS scanner_observations(ip TEXT PRIMARY KEY,family INTEGER NOT NULL,first_seen INTEGER NOT NULL,last_seen INTEGER NOT NULL,total_attempts INTEGER NOT NULL DEFAULT 0,last_nft_counter INTEGER NOT NULL DEFAULT 0,valid_ever INTEGER NOT NULL DEFAULT 0,valid_last_seen INTEGER,classification TEXT NOT NULL DEFAULT 'UNKNOWN',country_code TEXT DEFAULT '',city TEXT DEFAULT '',asn TEXT DEFAULT '',org TEXT DEFAULT '');
...
def classify(r,bans,white,now):
    if r['ip'] in bans:return 'BANNED'
    if r['ip'] in white:return 'WHITELIST'
    if r['valid_ever']:return 'CLIENT'
    n=int(r['total_attempts']); age=now-int(r['first_seen'])
    if (n>=10 and age>=60) or (n>=5 and age>=120):return 'SCAN'
    if n>=3 and age>=60 and hosting(r['org']):return 'HOSTING?'
    return 'UNKNOWN'
def collect():
    port=int(state().get('PORT','8443')); ensure(port); now=int(time.time()); ss=seen(); vv=valid()
    with con() as c:
        c.execute('UPDATE scanner_bans SET active=0,removed_at=? WHERE active=1 AND expires_at IS NOT NULL AND expires_at<=?',(now,now))
        ...
        for ip,count in ss.items():
            ...
            if r:
                ...
                c.execute('''UPDATE scanner_observations SET last_seen=?,total_attempts=total_attempts+?,last_nft_counter=?,valid_ever=CASE WHEN ? THEN 1 ELSE valid_ever END,valid_last_seen=CASE WHEN ? THEN ? ELSE valid_last_seen END,country_code=?,city=?,asn=?,org=? WHERE ip=?''',(now,max(0,delta),count,ip in vv,ip in vv,now,cc,city,asn,org,ip))
            else:
                cc,city,asn,org=geo(ip); c.execute('INSERT INTO scanner_observations(ip,family,first_seen,last_seen,total_attempts,last_nft_counter,valid_ever,valid_last_seen,country_code,city,asn,org) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',(ip,fam,now,now,max(count,1),count,1 if ip in vv else 0,now if ip in vv else None,cc,city,asn,org))
        for ip in vv:
            r=c.execute('SELECT ip FROM scanner_observations WHERE ip=?',(ip,)).fetchone()
            if r:c.execute('UPDATE scanner_observations SET valid_ever=1,valid_last_seen=? WHERE ip=?',(now,ip))
            else:
                fam=ipaddress.ip_address(ip).version; cc,city,asn,org=geo(ip); c.execute('INSERT INTO scanner_observations(ip,family,first_seen,last_seen,valid_ever,valid_last_seen,country_code,city,asn,org) VALUES(?,?,?,?,1,?,?,?,?,?)',(ip,fam,now,now,now,cc,city,asn,org))
        for r in c.execute('SELECT * FROM scanner_observations WHERE last_seen>=?',(now-30*86400,)).fetchall():c.execute('UPDATE scanner_observations SET classification=? WHERE ip=?',(classify(r,bans,white,now),r['ip']))
...
def ban(ip,dur,reason,actor):
    ...
    ensure(int(state().get('PORT','8443'))); addban_nft(ip,exp)
    with con() as c:c.execute("UPDATE scanner_observations SET classification='BANNED' WHERE ip=?",(ip,))
    audit('ban',ip,f'duration={label}; reason={reason}',actor); print(f'BANNED {ip} {label}')
def unban(ip,actor):
    ip=target(ip); delban_nft(ip); now=int(time.time())
    with con() as c:c.execute('UPDATE scanner_bans SET active=0,removed_at=? WHERE ip=?',(now,ip))
    audit('unban',ip,'',actor); print(f'UNBANNED {ip}')
def white(ip,note,actor):
    ip=target(ip); delban_nft(ip); now=int(time.time())
    with con() as c:
        c.execute('UPDATE scanner_bans SET active=0,removed_at=? WHERE ip=?',(now,ip)); c.execute('INSERT INTO scanner_whitelist VALUES(?,?,?,?) ON CONFLICT(ip) DO UPDATE SET note=excluded.note,actor=excluded.actor',(ip,now,note[:200],actor[:80])); c.execute("UPDATE scanner_observations SET classification='WHITELIST' WHERE ip=?",(ip,))
    audit('whitelist',ip,note,actor); print(f'WHITELIST {ip}')
def unwhite(ip,actor):
    ip=target(ip)
    with con() as c:c.execute('DELETE FROM scanner_whitelist WHERE ip=?',(ip,))
    audit('unwhitelist',ip,'',actor); print(f'UNWHITELIST {ip}')
def status(js=False):
    initdb(); now=int(time.time())
    with con() as c:
        h=c.execute("SELECT value FROM scanner_meta WHERE key='heartbeat'").fetchone(); cls={r[0]:r[1] for r in c.execute('SELECT classification,count(*) FROM scanner_observations WHERE last_seen>=? GROUP BY classification',(now-86400,))}; bans=c.execute('SELECT count(*) FROM scanner_bans WHERE active=1 AND (expires_at IS NULL OR expires_at>?)',(now,)).fetchone()[0]; wh=c.execute('SELECT count(*) FROM scanner_whitelist').fetchone()[0]
    d={'version':VERSION,'heartbeat':int(h[0]) if h else 0,'autoban':False,'bans':bans,'whitelist':wh,'classes_24h':cls}
...
def listing(kind):
    with con() as c:
        ...
        else:rs=c.execute("SELECT ip,classification,total_attempts,last_seen,country_code,city,asn,org FROM scanner_observations WHERE classification IN ('SCAN','HOSTING?','UNKNOWN') ORDER BY CASE classification WHEN 'SCAN' THEN 0 WHEN 'HOSTING?' THEN 1 ELSE 2 END,total_attempts DESC LIMIT 100").fetchall()
...
def main():
    ...
    s.add_parser('selftest');s.add_parser('init');s.add_parser('worker');q=s.add_parser('status');q.add_argument('--json',action='store_true');s.add_parser('suspicious');s.add_parser('bans')
    ...
    elif a.cmd in ('suspicious','bans'):listing(a.cmd)
